#!/bin/bash
# Bouw de screener-job, zet hem op Cloud Run en plan hem maandelijks.
...
SCHEDULE="0 6 1 * *"          # de eerste van de maand, 06:00 Amsterdam
...
echo "== build $IMAGE"
gcloud builds submit --project "$PROJECT" --config cloudbuild.screener.yaml .
...
echo "== maandelijkse trigger"
if gcloud scheduler jobs describe "$JOB-monthly" --project "$PROJECT" --location "$REGION" >/dev/null 2>&1; then
    ACTION=update
else
    ACTION=create
fi
gcloud scheduler jobs $ACTION http "$JOB-monthly" \
    --project "$PROJECT" \
    --location "$REGION" \
    --schedule "$SCHEDULE" \
